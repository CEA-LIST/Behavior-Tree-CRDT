...
__classifiers::record!(OutFlowPort { data_flow_port_feat : DataFlowPortFeatLog, });
__classifiers::record!(InFlowPort { data_flow_port_feat : DataFlowPortFeatLog, });
...
__classifiers::record!(Sequence { control_node_feat : ControlNodeFeatLog, });
__classifiers::record!(Fallback { control_node_feat : ControlNodeFeatLog, });
__classifiers::union!(
    Action = OpenDoor(OpenDoor, OpenDoorLog) | EnterRoom(EnterRoom, EnterRoomLog) |
    CloseDoor(CloseDoor, CloseDoorLog)
);
__classifiers::record!(ActionFeat { execution_node_feat : ExecutionNodeFeatLog, });
__classifiers::union!(Condition = IsDoorOpen(IsDoorOpen, IsDoorOpenLog));
__classifiers::record!(ConditionFeat { execution_node_feat : ExecutionNodeFeatLog, });
__classifiers::record!(
...
__classifiers::record!(Inverter { decorator_feat : DecoratorFeatLog, });
__classifiers::record!(IsDoorOpen { condition_feat : ConditionFeatLog, });
__classifiers::record!(OpenDoor { action_feat : ActionFeatLog, });
__classifiers::record!(EnterRoom { action_feat : ActionFeatLog, });
__classifiers::record!(CloseDoor { action_feat : ActionFeatLog, });
__classifiers::record!(
    SubTree { tree_node_feat : TreeNodeFeatLog, tree : BehaviorTreeLog, }
);
